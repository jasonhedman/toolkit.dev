...
if [ $? -eq 0 ]; then
  ...
  if [ -f ".env" ]; then
    ...
    if grep -q "^REDIS_URL=" .env; then
      ...
      echo "Updated REDIS_URL in .env file"
    else
    ...
    # Check if REDIS_URL already exists in .env.local
    if grep -q "^REDIS_URL=" .env.local; then
      ...
      echo "Updated REDIS_URL in .env.local file"
    else
      # Add REDIS_URL to .env.local
      echo "REDIS_URL=\"$REDIS_URL\"" >> .env.local
      echo "Added REDIS_URL to .env.local file"
    fi
  else
    echo "Warning: No .env or .env.local file found. Please manually add:"
