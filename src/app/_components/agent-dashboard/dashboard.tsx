...
import { RealtimeRunStatus } from "./realtime-run-status";
import { Play, History, Settings } from "lucide-react";

export const AgentDashboard = () => {
  const [activeTab, setActiveTab] = useState("configure");
  const [runStarted, setRunStarted] = useState<{ taskId: string; handle: any } | null>(null);

  const handleRunStarted = (taskId: string, handle: any) => {
    setRunStarted({ taskId, handle });
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <TabsContent value="configure" className="space-y-6">
          <Card className="p-6">
            ...
            <div className="mt-6">
              <AgentConfigurationForm onRunStarted={handleRunStarted} />
            </div>
          </Card>
          ...
          {runStarted && (
            <Card className="p-6">
              <div className="space-y-4">
                <h2 className="text-xl font-semibold">Live Agent Run</h2>
                <RealtimeRunStatus 
                  runId={runStarted.taskId} 
                  onComplete={() => {
